...
class HttpRequest {
  ...
  baseOptions(params: any, method = "GET", baseUrl = this.API_URL) {
    ...
    return Taro.request(option).catch(({ msg, code }: any) => {
      if (!ignoreErrorMsg) {
   ...
   * @return {Object} 返回 Promise 实例
   */
  call(callMethod: any, version = DEFAULT_VERSION, params: any, option: OptionsType) {
  ...
  callPost(callMethod: any, version = DEFAULT_VERSION, params: any, option: Object) {
   ...
   * @return {Object} 返回 Promise 实例
   */
  get<T>(url: any, data: T, options: OptionsType = {}) {
   ...
   * @return {Object} 返回 Promise 实例
   */
  post<T>(url: any, data: T, options: OptionsType = {}) {
   ...
   * @return {Object} 返回 Promise 实例
   */
  mockCall<T>(method: string, version = DEFAULT_VERSION, params: any, data: T) {
    let option = {
  ...
  mockCallPost<T>(method: string, version = DEFAULT_VERSION, params: any, data: T) {
    let option = {
      url: `/${method}/${version}?${stringify(params)}`,
      data,
    };
   ...
   * @return {Object} 返回 Promise 实例
   */
  mockPost<T>(url: string, data: T, options: OptionsType = {}) {
